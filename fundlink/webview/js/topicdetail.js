...
$(function(){
    ...
    $("#words_list ul").html("");
    if(token){
        $(".icon_sanjiao").click(function(){
            var flag = $(this).attr("data");
            if(flag == 0){
                $(".content p").css("-webkit-line-clamp","100");
                $(this).attr("data","1");
                $(this).css({"background":"url(images/icon_up.png) 100% no-repeat","background-size":"100% 100%"});
            }else{
                $(".content p").css("-webkit-line-clamp","4");
                $(this).attr("data","0");
                $(this).css({"background":"url(images/icon_down.png) 100% no-repeat","background-size":"100% 100%"});
            }
        });
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if(a.state == "审核中"){
                ...
                $(".icon_reply").click(function(){
                    $.cookie("replyUserId","");
                    $.cookie("historypage","1");// 0 活动详情 1话题详情 2文章详情
                ...
                $(".icon_zan").click(function(){
                    ...
                    if(flag == 0){
                        $(this).attr("data","1");
                        likesTopic(circleId,token,terminal,version);
                    }
                });
            }

            $(".head_title").html(a.title);
            ...
            if(a.lable != ""){
                ...
                $(".cus_label div").html("");
                $(".cus_label div").append(lable);
            }else {
                lable = "";
                $(".cus_label").hide();
            }

            $(".content p").html(a.context);
            //话题介绍文字长度
            var contentW = $(".content p").width();
            var fontS = $(".content p").css("font-size").substring(0,$(".content p").css("font-size").length-2)-0;
            var spill = Math.round(contentW/fontS)*4;
            if($(".content p").text().length>=spill){
                $(".icon_sanjiao").show();
            }else {
                $(".icon_sanjiao").hide();
            }
...
//话题点赞
function likesTopic(circleId,token,terminal,version){
    hideLoading();
    showLoading();
    $.ajax({
        type: 'post',
        timeout: 60000,
        url: ajaxUrl() + "likesTopic",
        data: {
            articleId: circleId
        },
        headers : {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $(".reply1").click(function(){
                ...
                $.cookie("historypage","1");// 0 活动列表
                location.href = "message.html"+locationSearch();
            });
            $(".delete").click(function(){
                var commentId = $(this).attr("commentId");
                var index = $(this).parents("li").index();
                delShowAlert(commentTopic,commentId,index,token,terminal,version);
            })
        },
        error: function(data){
            hideLoading();
...
//删除评论
function commentTopic(commentId,index,token,terminal,version){
    hideLoading();
    showLoading();
    $.ajax({
        type: 'delete',
        timeout: 60000,
        url: ajaxUrl() + "commentTopic?commentId=" + commentId,
        /*data: {
            commentId: commentId
        },*/
        headers : {
